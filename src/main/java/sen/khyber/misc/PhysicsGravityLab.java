...
public class PhysicsGravityLab {
    ...
    private final double frontHeight,
            backHeight,
            distGateToTable,
            paperDist,
            gateTime;
    ...
    public PhysicsGravityLab(final double frontHeight, final double backHeight,
            final double distGateToTable, final double paperDist, final double gateTime) {
    ...
    private double calcGravity() {
        final double fallTime = fallTime();
        return 2 * (vertDist() - vertVelocity() * fallTime) / (fallTime * fallTime);
    }
    
    public double percentError1(final double ballDiameter) {
        this.ballDiameter = ballDiameter;
        final double percentDecimal = (GRAVITY - calcGravity()) / GRAVITY;
        final int truncatedPercent = (int) (percentDecimal * 100000);
        return truncatedPercent / 100000.0;
    }
    
    public List<Double> enumeratePercentErrors1(final double minBallDiameter, final double step) {
        final int numTests = (int) ((MAX_BALL_DIAMETER - minBallDiameter) / step);
        final List<Double> percentErrors = new ArrayList<>(numTests);
    ...
    private static double calcBallDiameter(final double distFromDiameter) {
        final double chord = 2 * Math.sqrt(RADIUS * RADIUS -
                distFromDiameter * distFromDiameter);
        //System.out.println(chord);
        return chord;
    }
    
    public double percentError2(final double distFromDiameter) {
        return percentError1(calcBallDiameter(distFromDiameter));
    }
    
    public List<Double> enumeratePercentErrors2(double maxDistFromDiameter, final double step) {
        if (maxDistFromDiameter > RADIUS) {
            maxDistFromDiameter = RADIUS;
        }
        final List<Double> percentErrors = new ArrayList<>();
    ...
    public static void main(final String[] args) throws Exception {
        final PhysicsGravityLab trial1 = new PhysicsGravityLab(0.0565, 0.2550, 0.034, 0.1607,
                0.0150);
        final PhysicsGravityLab trial2 = new PhysicsGravityLab(0.0635, 0.1825, 0.000, 0.1184,
                0.0216);
        final PhysicsGravityLab trial3 = new PhysicsGravityLab(0.0760, 0.1145, 0.000, -0.0073,
                0.0271);
        final PhysicsGravityLab trial4 = new PhysicsGravityLab(0.0620, 0.3140, 0.000, 0.1992,
                0.0100);
        final PhysicsGravityLab[] trials = new PhysicsGravityLab[] {trial1, trial2, trial3, trial4};
        final double[][] allPercentErrors = new double[trials.length][];
        for (int i = 0; i < trials.length; i++) {
            final List<Double> percentErrorsList = trials[i].enumeratePercentErrors1();
            final double[] percentErrors = new double[percentErrorsList.size()];
        ...
        for (final PhysicsGravityLab trial : trials) {
